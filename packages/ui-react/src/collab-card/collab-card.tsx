...
export interface CollabCardProps {
	...
	liveLabel?: string;
	/** Background image URL. Falls back to a dark canvas gradient. */
	backgroundUrl?: string;
	className?: string;
}

function PresenceStack({ colors, extraCount }: { colors: string[]; extraCount: number }) {
...
function Cursor({ className }: { className?: string }) {
	return (
		<svg
			...
			aria-hidden
			className={cn(
				"h-[6.5cqi] w-[6.5cqi] drop-shadow-[0_2px_4px_rgba(0,0,0,0.28)]",
				className,
			)}
		>
			<path
			...
			<div className="relative flex h-full w-full flex-col items-center justify-center gap-[3.2cqi] px-[5.5cqi] pb-[5cqi] pt-[9cqi]">
				...
				<div className="relative w-[80%] max-w-full">
					<div className="relative grid place-items-center rounded-[1.8cqi] border-2 border-dashed border-[#0D99FF]/75 px-[5.5cqi] py-[4.2cqi]">
						...
						<h2 className="font-(family-name:--font-heading) text-[17.5cqi] font-medium leading-[0.92] tracking-[-0.035em] text-white">
							{greeting}
						</h2>
					</div>
