...
/** The fill is always full size; clip-path reveals only the icon tile until hover or focus. */
export const fillButton = tv({
	slots: {
		root: [
			"group/fill relative isolate inline-flex shrink-0 select-none items-center overflow-hidden whitespace-nowrap bg-primary p-1 font-medium text-primary-foreground",
		...
		fill: [
			"pointer-events-none absolute inset-1 -z-10 [clip-path:inset(0_calc(100%-var(--fill-tile))_0_0_round_var(--fill-radius))]",
		...
		icon: "grid h-full w-(--fill-tile) shrink-0 place-items-center",
		label: [
		...
		fillLabel: [
			"pointer-events-none absolute inset-y-1 left-1 flex -translate-x-full items-center pl-(--fill-gap) opacity-0 transition-[translate,opacity]",
			EASE,
	...
	variants: {
		tone: {
			accent: {
				...
				fillLabel: "text-(--accent-foreground)",
			},
			inverse: {
				fill: "bg-primary-foreground",
				icon: "text-primary",
				fillLabel: "text-primary",
			},
		},
		size: {
			sm: {
