...
export const pricing02 = tv({
	slots: {
		...
		title:
			"mt-4 max-w-xl whitespace-pre-line text-balance font-medium text-4xl leading-[1.02] tracking-tight sm:text-5xl",
		aside: "flex flex-col items-start gap-5 md:col-span-4 md:col-start-9",
		description: "max-w-md text-muted-foreground text-sm leading-6",
		plans: "mt-12 grid gap-3 md:grid-cols-[repeat(auto-fit,minmax(16rem,1fr))]",
		...
		list: "flex flex-1 flex-col px-4 pt-8 pb-5 sm:px-5",
		listLabel:
		...
		footnotes:
			"mt-5 flex flex-col gap-2 text-muted-foreground text-xs sm:flex-row sm:items-center sm:justify-between",
	},
	variants: {
		variant: {
