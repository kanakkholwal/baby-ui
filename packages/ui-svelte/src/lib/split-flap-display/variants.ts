...
// Fit: a row is about 1.3em per column plus 2.4em of padding and strips.
export const splitFlap = tv({
	slots: {
		root: "@container w-full font-bold font-mono",
		board: "mx-auto flex w-fit max-w-full flex-col rounded-[0.6em] px-[0.6em] pt-[0.3em] pb-[0.6em] transition-[font-size] duration-(--duration-dropdown) ease-(--ease-out) motion-reduce:transition-none",
		rowShell:
		...
		glyphTop: "absolute inset-x-0 top-0 flex h-[200%] items-center justify-center leading-none",
		glyphBottom:
		...
		divider: "pointer-events-none absolute inset-x-0 top-1/2 z-20 h-[max(1px,0.05em)] -translate-y-1/2",
	...
	variants: {
		variant: {
			solid: {
				...
				bottom: "bg-linear-to-b from-[color-mix(in_oklab,var(--foreground)_81%,var(--background))] to-[color-mix(in_oklab,var(--foreground)_77%,var(--background))]",
			...
			card: {
				...
				bottom: "bg-linear-to-b from-[color-mix(in_oklab,var(--card)_92%,var(--foreground))] to-[color-mix(in_oklab,var(--card)_88%,var(--foreground))]",
				divider: "bg-border",
			},
		},
		size: {
			sm: {
				board: "text-[length:min(16px,calc(100cqi/(var(--split-flap-columns)*1.3_+_2.4)))]",
			},
			md: {
				board: "text-[length:min(22px,calc(100cqi/(var(--split-flap-columns)*1.3_+_2.4)))]",
			},
			lg: {
				board: "text-[length:min(30px,calc(100cqi/(var(--split-flap-columns)*1.3_+_2.4)))]",
			},
		},
		indicator: {
			none: {},
