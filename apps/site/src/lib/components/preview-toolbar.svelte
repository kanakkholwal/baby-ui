...
function act(action: string, run: () => void) {
	run();
	track("preview_action", { action });
}
</script>
...
<div class="flex shrink-0 items-center gap-0.5 rounded-full border border-border bg-card/20 p-1">
	<button
		...
		onclick={() => act("desktop", () => (viewport = "desktop"))}
		class={button}
	>
	...
	<button
		...
		onclick={() => act("mobile", () => (viewport = "mobile"))}
		class={button}
	>
		<IconDeviceMobile size={15} stroke={1.6} />
	</button>
	<button type="button" aria-label="Reload preview" onclick={() => act("reload", onReload)} class={button}>
		<IconRefresh size={15} stroke={1.6} />
	</button>
	<button
		type="button"
		aria-label={fullscreen ? "Exit fullscreen" : "Fullscreen preview"}
		onclick={() => act(fullscreen ? "exit_fullscreen" : "fullscreen", () => (fullscreen = !fullscreen))}
		class={button}
	>
		{#if fullscreen}
