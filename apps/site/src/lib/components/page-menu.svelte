...
const ask = $derived(encodeURIComponent(`Read ${absolute} and help me with it.`));

// Brand marks as shadcn's docs draw them; Tabler has no v0 or Claude glyph.
const MARKDOWN = {
	viewBox: "0 0 22 16",
	evenodd: true,
...
async function copyPage() {
	const res = await fetch(markdownUrl).catch(() => null);
	await navigator.clipboard.writeText(res?.ok ? await res.text() : copyText);
	copied = true;
	clearTimeout(timer);
...
{#snippet mark(icon: { viewBox: string; d: string; evenodd?: boolean })}
	<svg viewBox={icon.viewBox} fill="currentColor" aria-hidden="true" class="size-3.5 shrink-0">
		<path d={icon.d} fill-rule={icon.evenodd ? "evenodd" : undefined} />
	</svg>
{/snippet}
